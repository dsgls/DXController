...
- **Unconditional** (plain `Log(...)` at the call site) — the baseline every
  user ticket log should contain: the launcher's controller
  detection/hotplug/active-device lines, `ControllerRootWindow`'s hooking
  events (direct-child `DXC-NAV DESC-ADD`, `TICK-TOP`, `TICK-INIT`),
  cursor-mode switches (`DXC-CURSOR`) and slow frames (`DXC-PERF`). Reserve
  this tier for rare, edge-triggered messages that confirm the integration
  is alive or record a device handoff.
- **`bNavDebugLog`**, via `class'DXControllerDebug'.static.NavLog("DXC-…")` —
  UI/navigation diagnostics (`DXC-NAV` focus/activate traces, `DXC-TERM`,
  `DXC-CONV`, `DXC-KBD`, `DXC-WHEEL`, `DXC-VKP`, `DXC-FOCUS`).
- **`bGamepadDebugLog`**, via `.static.DebugLog(...)` — input-layer detail
  (`DXC-AXIS`; the launcher also reads this flag for its device-idle edges).
