...
`ButtonMapResolve`, `AxisMapParse`, `PadActivity`, `PadNavGraph`,
`PadRepeat`.
...
- **`Engine/Classes/Console.uc` carries its own local copy of `EInputKey`**
  ...
  for D-pad up).
- **A class-scoped `enum` tag is not globally visible — qualify it through
  an object as `obj.EnumType.Tag`.** An `enum` declared inside a class (e.g.
  `EGameDirectoryTypes { GD_Maps, GD_SaveGames }` in `GameDirectory`) is in
  scope only within that class. From any other class the bare tag
  `GD_SaveGames` fails to resolve; reach it through an instance of the
  declaring class:
  `saveDir.SetDirType(saveDir.EGameDirectoryTypes.GD_SaveGames)`. Stock has
  zero bare-tag uses across packages — see
  `../deusex-scripts/DeusEx/Classes/MenuScreenLoadGame.uc:750` and
  `LoadMapWindow.uc:27`. (Contrast `EInputKey`, reachable unqualified only
  because it's declared on the widely-inherited `Actor`/`Console` base.)
...
- **Conversation windows `Hide()`/`Show()` mid-dialog, transiently
  detaching the nav controller.** `Hide()` unlinks the window from the
  root's child list, so `FindTopmostModalNav` reports no modal for a
  frame and Tick reconciliation detaches/re-attaches
  `ConversationNavController` around the gap. Corollary: do not park
  per-conversation state in fields set from `Attach`/`Detach` — it goes
  stale inside the gap. Concrete instance: a `bGamepadMode` flag on the
  `ConWindowActive` overlay was cleared by the transient detach, letting
  `AddButton`'s vanilla `root.ShowCursor(True)` re-show the cursor
  mid-conversation. Query live state at call time instead — the
  `ShowCursor` gates now call `root.IsGamepadCursorMode()`, a
  `DeusExRootWindow` hook overridden in `ControllerRootWindow`.
...
- **`Window.SetFocusWindow(w)` does not make a HUD-style root child the
  ...
  `DXController` type).
- **`DSTY_Translucent` is additive — texture luminance IS opacity; black
  adds nothing and can never darken.** The formula is `framebuffer +=
  texel * tileColor`, so a texel with RGB (0,0,0) contributes zero
  regardless of tile colour. To use a greyscale texture as a shaped
  tinted fill: keep the art on a pure-black background, draw
  `DSTY_Translucent`, and set the tile colour to the desired tint.
  Stock item-bar (HUDObjectBelt) uses this: measured luminances are **50**
  for cell interiors (`HUDObjectBeltBackground_Cell`) and **75** for
  frames (bar fill / cell edges). The DXController UI (weapon wheel, OSK
  panel) derives tile colours from those same constants so the widgets
  follow the player's HUD theme colour. Stacked translucent draws sum —
  drawing a cell fill (lum 50) on top of a panel fill (also lum 50) adds
  to an effective 100. Constants and helpers live in
  `DXController/Classes/DXCUITheme.uc`.
- **No `GC` blend style gives a uniform translucent fill — combine two.**
  ...
  *additive* (`framebuffer += texel * tileColor`, ignores tile alpha),
  ...
  (`dest * texel * 2`, texel `128` = identity) and **ignores the GC
  tile colour entirely** — UE1 render devices force modulated polys to
  identity colour, so the darkening strength must live in the
  *texture's texel value*. Evidence: stock `ConWindow.uc` sets
  `SetTileColorRGB(0,0,0)` yet the conversation letterbox is the ×0.5
  dim of its flat-grey-64 `ConWindowBackground` texture, not black;
  `DeusExScopeView.uc` never sets a tile colour before its modulated
  scope draws; and `Texture'Solid'` is flat *white* 255, so a
  modulated draw over `Solid` is `dest × 2` — a **brighten**, whatever
  tint you set (this bug shipped in the first OSK/hint-overlay veils).
  For an *even* dark veil, draw twice: `DSTY_Modulated` over a flat
  dark texture (`Texture'DXController.Veil'`, flat 32 = ×0.25,
...
- **For a translucent-tinted glow over arbitrary geometry, use
  ...
  Consequence for textures that serve *both* masked and translucent
  paths (e.g. `WheelPlate`): the engine masks by palette index 0, not
...
- **UE1 does NOT null `Object`/`Window` references when the target is
  ...
  slot is reused).** Only `Actor` references *stored in level actors*
  get swept to `None` on destruction (`ULevel::CleanupDestroyed` runs
  the nulling pass over `Level->Actors[]` only — actor refs held by
  windows or any other non-actor `Object` are never nulled; see the
  mission-travel entry below). `Window` extends `Object`, not `Actor`,
  so a held pointer stays non-`None` and points at freed (or recycled)
  memory.
  ...
  **Corollary: when vanilla rebuilds a subtree in place (destroy +
  recreate, same screen), NO pointer probe can detect it — not even
  the liveness walk.** The freed slots are immediately reused by the
  replacement windows, so a stale pointer compares equal to a *new
  live* window and every check passes while focus silently points at
  the wrong widget (the medbot aug-install bug: after
  `InstallAugmentation → PopulateAugCanList`, the cached "right
  column" button aliased the new left button, reversing D-pad nav;
  the not-reused pointers crashed in `GetClientObject` once the heap
  churned). The only reliable detector is event-driven invalidation:
  `ControllerRootWindow` pumps `DescendantAdded`/`DescendantRemoved`
  into the active controller's `OnScreenDescendantAdded/Removed`
  hooks; a controller that caches subtree pointers marks them dirty
  there (filter by descendant class) and re-collects from the live
  tree on next use — deferred, because the Added hook fires before
  vanilla has filled the new windows' client data. See
  `AugInstallNavController`.
- **Mission travel hard-deletes the player's aug/skill actors, then
  paints the old window tree once more for the loading screen — a
  window still drawing from cached actor refs at that paint
  dereferences freed memory.** Sequence (verified in the binaries —
  see `../deusex-native-re/`): `ClientTravel` → engine
  `SetClientTravel` dispatches `PreTravel()` synchronously to every
  level actor; on the next engine Tick DeusEx's `Browse` runs
  `PruneTravelActors` (destroys every `Augmentation` + its manager,
  every `Skill` + manager — so they aren't serialized into the level
  being saved for re-entry) and `SaveCurrentLevel` (forced
  `CleanupDestroyed` — actor memory is freed, and the ref-nulling pass
  covers level actors only, never windows); `LoadMap` then immediately
  paints the loading screen through the OLD window tree
  (`PaintProgress` → `PostRenderFlash` → `DrawWindow`s). Crashed live:
  aug wheel held open through a map exit GPF'd inside
  `RadialMenuWindow.DrawWindow` on freed `augSlots` entries. The
  script-side hook that fires early enough is `DeusExPlayer.
  PreTravel()` → `DeusExRootWindow.PreTravelNotify()` (overlay no-op
  hook) → `ControllerRootWindow.PreTravelNotify()` →
  `RadialMenuWindow.OnPreTravel()`, which closes the wheel and nulls
  its cached actor refs. Any future HUD window that caches actor refs
  must hook the same chain. (Plain load-game / new-game paints are
  safe — the old level's actors are still alive when that loading
  screen is painted.)
...
- **`PersonaScreenInventory.bDragging` gates the 0.25s item-button
  rebuild — set it to suppress the rebuild during a modal grid
  mutation, and restore `invSlots` on teardown.** `RefreshWindow`
  rebuilds every `winItems` button (and runs `CleanBelt`) each 0.25s
  but only `if (!bDragging)`. A modal operation that holds a `focused`
  button pointer and temporarily clears an item's `invSlots` (the
  gamepad move mode — `InvNavController` `subDialogActive == 'Move'`,
  the mouse drag's script-side analogue) must set `bDragging = True` on
  enter so the rebuild can't destroy the button or refill the cleared
  cells mid-operation, and clear it on every exit. Crucially the
  player's `invSlots` grid lives on the pawn and persists when the menu
  closes, so if the operation can be interrupted by a menu-close
  (Back/Start bypass the in-screen cancel), it must restore the cleared
  footprint from `Detach()` — which still runs against a live `screen`
  pointer — or the next pickup's slot search sees phantom-free cells.
  See `InvNavController.EnterMove` / `Detach`.
- **Textures compile into `DXController.u` via an `#exec` holder, not a
  ...
  (`FLAGS=2` = PF_Masked, keys palette index 0), and non-masked
  greyscale wedges (`Wedge0..Wedge9`, no FLAGS — for additive
  `DSTY_Translucent` draws). Textures over 256px **must** import
  `MIPS=On`: render devices walk down the mip chain when a texture
  exceeds the device cap, and stock D3DDrv caps at 256x256 and
  appErrors when no mip fits. Masked + mips is safe — the importer's
  mip filter excludes the key from the blend, so `MIPS=On` never
  bleeds the key colour. `FILE=` paths are **relative to the package
...
- **`Float` is a 32-bit float — large numeric sort keys silently lose
  precision.** Stock `MenuScreenLoadGame.BuildTimeJulian` packed the
  save timestamp into a Float (~1.36e9 in 2026, where adjacent
  representable values are 128 apart), so saves made within ~2 minutes
  collided on the load list's hidden date-sort column; the native
  `XListWindow` tie-break (row insertion order = save-slot order) then
  listed the older save as newest. Fixed in our overlay
  (`DeusEx/Classes/MenuScreenLoadGame.uc`) with a fixed-width
  `YYYYMMDDHHMMSS` string key in a `COLTYPE_String` column. If a list
  needs an exact numeric sort key, keep its magnitude under 2^24 or
  sort a fixed-width digit string instead.
- **The console renders straight to the raw `UCanvas`, bypassing the
  launcher's GUI scaling fix entirely.** That fix
  (`launcher/src/GUIScalingFix.cpp`) only rewrites
  `XRootWindow.hMultiplier/vMultiplier`, so it scales the window-system
  tree (HUD, menus, conversations). `Console.PostRender` is a separate
  engine path drawn directly onto the viewport canvas — it never touches
  `XRootWindow` — so console text stays at native pixel size and reads
  microscopic at high resolution. `Canvas.DrawText` is `final native`
  with **no scale parameter** (verified in `Engine/Classes/Canvas.uc`),
  so the only lever for canvas text size is which `Font` you assign.
  Stock `DrawSingleView` (the "press T" typing line) and
  `DrawConsoleView` (scrollback) both draw with `C.MedFont`. To enlarge
  console text without reimplementing those large stock draw functions,
  override `PostRender` and temporarily repoint `C.MedFont` to a bigger
  font (`C.LargeFont`, the engine's Height=32 TrueType) across the
  `Super.PostRender` call, restoring it after — see
  `ControllerConsole.PostRender`. Note `Deus Ex` leaves `myHUD` unset
  (its HUD is the window system, not a `HUD` actor), so the
  `DrawSingleView` early-return at `Console.uc:523` never fires and the
  typing block always runs.

## Debug logging

Gamepad-domain logs are split into three tiers:

- **Unconditional** (plain `Log(...)` at the call site) — the baseline every
  user ticket log should contain: proof the integration is hooked, device
  handoffs and detection, abnormal states. Reserve this tier for rare,
  edge-triggered messages — at most one line per screen change, never
  per-event.
- **`bNavDebugLog`**, via `class'DXControllerDebug'.static.NavLog("DXC-…")` —
  UI/navigation diagnostics.
- **`bGamepadDebugLog`**, via `.static.DebugLog(...)` — input-layer detail
...
sibling `var config bool` to `DXControllerDebug.uc` plus a parallel helper.
Prefix all such logs with `DXC-<area>`. Enable via `DeusEx.ini`:
