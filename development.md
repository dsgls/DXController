...
- **No `GC` blend style gives a uniform translucent fill — combine two.**
  ...
  *additive* (`framebuffer += texel * tileColor`, ignores tile alpha),
  ...
  dark floor back. Modulated identity: tile grey `128` = ×1.0 (scene
  unaffected), `32` ≈ ×0.25 (strong darkening). Precedents: stock
  `ConWindow.uc` (conversation letterbox, `SetTileColorRGB(0,0,0)` =
  ×0 to pure black) and `ControllerHintOverlay.uc` (grey 24 = ×0.19,
  balanced by an additive floor of 26). See
  `OnScreenKeyboardWindow.DrawWindow`.
