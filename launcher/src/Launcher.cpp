...
void CLauncher::MainLoop(UEngine* const pEngine)
{
    ...
    while (GIsRunning && !GIsRequestingExit)
    {
        ...
        if(m_pViewPort)
        {
            ...
            //The game's own cursor centering (SetMouseCapture) never runs under raw
            //input: it rides on WM_MOUSEMOVE, which the raw-input path blocks.
            //SetCursorPos still works, hence the sync below. Menu mouse input uses
            //relative deltas (MouseDelta, not MousePosition), so the rest of the
            //missing centering does not matter.
